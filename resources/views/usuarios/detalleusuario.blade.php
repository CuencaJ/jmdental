    ...
    <main class="flex-1 overflow-y-auto p-8 lg:p-10">
        ...
        <div class="mb-6">
            <a href="{{ Auth::user()->hasRole('administrador') 
                ...
                : (Auth::user()->hasRole('recepcionista') ? route('recepcionista.pacientes') : route('odontologo.pacientes.index')) }}"
        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            ...
            <div class="flex flex-wrap gap-3">
                @if($usuario->hasRole('paciente'))
                    ...
                    {{-- Descargar Resumen --}}
                    <a href="{{ Auth::user()->hasRole('administrador')
                        ? route('admin.pacientes.resumen', $usuario->id)
                        : (Auth::user()->hasRole('recepcionista') ? route('recepcionista.pacientes.resumen', $usuario->id) : route('odontologo.pacientes.resumen', $usuario->id)) }}"
                        target="_blank"
                    ...
                    @php
                        ...
                        $rutaAgendar = Auth::user()->hasRole('administrador')
                            ? route('admin.citas.create', ['paciente_id' => $pacienteId])
