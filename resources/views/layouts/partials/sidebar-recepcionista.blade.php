<aside class="w-64 flex flex-col bg-white border-r border-slate-200" style="min-height: 100vh">
    ...
    <div class="flex items-center gap-3 p-3 mx-4 bg-slate-50 rounded-xl mb-4">
        ...
        <div class="flex flex-col overflow-hidden">
            ...
            <p class="text-xs text-slate-500">Recepcionista</p>
        </div>
    </div>

    <nav class="flex-1 px-4 space-y-1">
        <a class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
            {{ request()->routeIs('recepcionista.dashboard') ? 'bg-blue-50 text-blue-500 font-semibold' : 'text-slate-600 hover:bg-slate-100' }}"
            href="{{ route('recepcionista.dashboard') }}">
            <span class="material-symbols-outlined">dashboard</span>
            <span class="text-sm">Inicio</span>
        </a>
        <a class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
            {{ request()->routeIs('recepcionista.citas*') ? 'bg-blue-50 text-blue-500 font-semibold' : 'text-slate-600 hover:bg-slate-100' }}"
            href="{{ route('recepcionista.citas') }}">
            <span class="material-symbols-outlined">calendar_month</span>
            <span class="text-sm">Citas</span>
        </a>
        <a class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
            {{ request()->routeIs('recepcionista.pacientes*') ? 'bg-blue-50 text-blue-500 font-semibold' : 'text-slate-600 hover:bg-slate-100' }}"
            href="{{ route('recepcionista.pacientes') }}">
            <span class="material-symbols-outlined">group</span>
            <span class="text-sm">Pacientes</span>
        </a>
        <a class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors
            {{ request()->routeIs('recepcionista.pacientes.create') ? 'bg-blue-50 text-blue-500 font-semibold' : 'text-slate-600 hover:bg-slate-100' }}"
            href="{{ route('recepcionista.pacientes.create') }}">
